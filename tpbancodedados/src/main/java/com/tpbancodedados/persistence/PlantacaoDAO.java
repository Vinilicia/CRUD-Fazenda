...
public class PlantacaoDAO {
    ...
    public int inserirPlantacao(Plantacao plantacao) {
        String query = "INSERT INTO Plantacao (id_funcionario, cultura, area, data_plantio) VALUES (?, ?, ?, ?)";
        int idGerado = -1;
        ...
        try (Connection connection = DatabaseConnection.getConnection();
			...
			System.out.println(plantacao.getDataPlantio());
    ...
    public List<Plantacao> listarPlantacoes() {
        ...
        try (Connection connection = DatabaseConnection.getConnection();
            ...
            while (resultSet.next()) {
                ...
                // Verificar se a data_colheita é NULL antes de converter para LocalDate
                Date dataColheita = resultSet.getDate("data_colheita");
                if (dataColheita != null) {
                    plantacao.setDataColheita(dataColheita.toLocalDate());
                } else {
                    plantacao.setDataColheita(null); // Deixar como null caso seja NULL no banco
                }
    ...
    public boolean atualizarPlantacao(Plantacao plantacao) {
		...
		if (plantacao.getDataColheita() != null) {
			query = "UPDATE Plantacao SET id_funcionario = ?, cultura = ?, area = ?, data_plantio = ?, data_colheita = ? WHERE id_plantacao = ?";
		} else {
			query = "UPDATE Plantacao SET id_funcionario = ?, cultura = ?, area = ?, data_plantio = ? WHERE id_plantacao = ?";
		}
